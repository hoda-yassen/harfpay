...
// تقصير عناوين مقالات كانت أطول من ٦٠ حرف (بتتقطع في نتائج بحث جوجل)، وتفريق الكلمات المفتاحية
// بين مقالات "الربح من الذكاء الاصطناعي" اللي كانت متكدسة على نفس الكلمات وبتنافس بعضها بدل ما تقوّي بعض.
(function fixArticleSeoTitlesAndKeywords() {
  const FLAG = 'seo_titles_keywords_fixed_v1';
  if (db.prepare('SELECT 1 FROM app_meta WHERE key = ?').get(FLAG)) return;

  const titleFixes = {
    'burnout-recovery-guide': 'الإرهاق الوظيفي: علاماته المبكرة وخطوات التعافي منه',
    'ai-writing-guide': 'كيف تستخدم ChatGPT في الكتابة دون أن تفقد بصمتك؟',
    'social-virality-guide': 'خوارزمية السوشيال ميديا 2026: كيف ينتشر مقالك؟',
    'profit-from-ai-2026': 'الربح من الذكاء الاصطناعي 2026: 7 طرق مجربة لدخل حقيقي',
  };
  const upTitle = db.prepare('UPDATE articles SET title = ? WHERE slug = ?');
  for (const [slug, title] of Object.entries(titleFixes)) upTitle.run(title, slug);

  const keywordFixes = {
    'profit-from-ai-2026': 'الربح من الذكاء الاصطناعي 2026, طرق ربح من AI, دخل شهري من الذكاء الاصطناعي, أفضل طرق كسب المال بالذكاء الاصطناعي',
    'chatgpt-money-guide': 'الربح من ChatGPT للمبتدئين, العمل بالذكاء الاصطناعي بدون خبرة برمجة, كيف تبدأ الربح من ChatGPT, أدوات الذكاء الاصطناعي للمبتدئين',
    'best-ai-side-hustles': 'مشاريع ذكاء اصطناعي مربحة 2026, أفكار مشاريع صغيرة بالذكاء الاصطناعي, استثمار في الذكاء الاصطناعي, مشاريع AI للمبتدئين',
  };
  const upKw = db.prepare('UPDATE articles SET meta_keywords = ? WHERE slug = ?');
  for (const [slug, kw] of Object.entries(keywordFixes)) upKw.run(kw, slug);

  db.prepare('INSERT OR REPLACE INTO app_meta (key, value) VALUES (?, ?)').run(FLAG, new Date().toISOString());
})();

// ٣ مقالات احترافية جديدة لحساب أحمد عوض الله (مترجم ومعلم إنجليزي)، محسّنة لمحركات البحث
// (عناوين تحت ٦٠ حرف، وصف ووسوم مفتاحية مميزة لكل مقال بدون تداخل بينها) — تُضاف مرة واحدة فقط.
(function seedAhmedTranslationArticles() {
  const FLAG = 'ahmed_translation_articles_added_v1';
  if (db.prepare('SELECT 1 FROM app_meta WHERE key = ?').get(FLAG)) return;

  const ahmed = db.prepare("SELECT id FROM users WHERE email = 'ahmed.awadallah@harf.demo'").get();
  if (!ahmed) return;

  const getCategoryId = (slug) => { const c = db.prepare('SELECT id FROM categories WHERE slug = ?').get(slug); return c ? c.id : null; };
  ...
  const insertArticle = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 'published', 1, datetime('now'))
  `);

  const articles = [
    {
      category: getCategoryId('language'),
      title: 'دليل شامل لاحتراف الترجمة من الإنجليزية للعربية',
      slug: 'english-arabic-translation-guide',
      description: 'دليل عملي لكل من يريد احتراف الترجمة من الإنجليزية للعربية، من أساسيات الترجمة إلى أدوات العمل الحر والتخصص المهني.',
      metaKeywords: 'احتراف الترجمة, الترجمة من الإنجليزية للعربية, تعلم الترجمة, مهارات المترجم المحترف',
      image: 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?auto=format&fit=crop&w=1200&q=80',
      minutes: 6,
      content: `تعتبر الترجمة من أكثر المهن التي يزداد الطلب عليها عامًا بعد عام، خاصة مع انفتاح العالم الرقمي وتوسع المحتوى العابر للغات. لكن الترجمة ليست مجرد نقل كلمات من لغة إلى أخرى، بل هي مهارة تتطلب فهمًا عميقًا للغتين، وحسًا ثقافيًا، وقدرة على نقل المعنى والروح لا الألفاظ فقط.

الخطوة الأولى نحو احتراف الترجمة هي إتقان اللغتين المصدر والهدف بشكل حقيقي، لا يقتصر على القواعد والمفردات، بل يمتد إلى فهم التعبيرات الاصطلاحية والسياقات الثقافية التي تختلف من لغة لأخرى. كثير من المترجمين المبتدئين يقعون في فخ الترجمة الحرفية، التي قد تكون صحيحة لغويًا لكنها غريبة أو غير مفهومة في اللغة الهدف.

التخصص هو المفتاح الثاني لبناء مسيرة مهنية ناجحة في الترجمة. سوق الترجمة واسع جدًا، ويشمل الترجمة القانونية، والطبية، والتقنية، والأدبية، والتسويقية، وغيرها. المترجم الذي يختار تخصصًا محددًا ويتعمق فيه، يصبح مرجعًا في مجاله، ويستطيع طلب أسعار أعلى مقارنة بالمترجم العام الذي يترجم في كل المجالات دون تعمق حقيقي في أي منها.

من الناحية العملية، هناك عادات يومية تساعد أي مترجم طموح على تطوير مهاراته:

أولًا، القراءة المستمرة باللغتين. المترجم الجيد قارئ نهم، يتابع المحتوى الحديث بكلا اللغتين، ما يجعله على اطلاع دائم بالتطورات اللغوية والمصطلحات الجديدة.

ثانيًا، الترجمة اليومية كتدريب. حتى لو لم تكن هناك مشاريع فعلية، ترجمة نص قصير يوميًا من الأخبار أو من كتاب، تصقل المهارة وتبني سرعة الأداء تدريجيًا.

ثالثًا، مراجعة الترجمات القديمة. العودة لترجمة قديمة بعد فترة وإعادة تقييمها بعين ناقدة، من أفضل الطرق لملاحظة التطور ونقاط الضعف المتكررة.

رابعًا، استخدام أدوات الترجمة بمساعدة الحاسوب، والتي تساعد على تنظيم المشاريع الكبيرة والحفاظ على اتساق المصطلحات عبر النصوص الطويلة.

أما عن بناء مسيرة مهنية فعلية، فالمترجم المستقل يحتاج لبناء أرشيف أعمال يعرض تخصصه ومستوى جودته، سواء عبر منصات العمل الحر العالمية، أو عبر النشر المباشر لعينات ترجمة على منصات المحتوى.

من الأخطاء الشائعة التي يقع فيها المترجمون المبتدئون، قبول أي مشروع دون النظر لملاءمته لتخصصهم، ما يؤدي لترجمات متوسطة الجودة تضر بالسمعة المهنية على المدى الطويل. الأفضل دائمًا التركيز على عدد محدود من المجالات وإتقانها، بدلًا من التوسع الأفقي غير المدروس.

في النهاية، الترجمة مهنة تجمع بين الفن والتقنية، ومن يستثمر في تطوير أدواته اللغوية والمهنية باستمرار، يجد لنفسه مكانًا مطلوبًا في سوق يزداد اتساعًا يومًا بعد يوم، خاصة مع تزايد الحاجة لمترجمين بشريين قادرين على فهم السياق والفروق الدقيقة التي لا تزال تتجاوز قدرات الترجمة الآلية.`,
    },
    {
      category: getCategoryId('education'),
      title: 'أفضل الطرق لتعلم اللغة الإنجليزية للمبتدئين في 2026',
      slug: 'learn-english-2026-guide',
      description: 'أفضل الاستراتيجيات العملية لتعلم اللغة الإنجليزية من الصفر، مع نصائح مجربة من معلم متخصص لتسريع التقدم وتثبيت المهارة.',
      metaKeywords: 'تعلم اللغة الإنجليزية, طرق تعلم الإنجليزية للمبتدئين, تحسين المحادثة بالإنجليزية, تعلم الإنجليزية 2026',
      image: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&w=1200&q=80',
      minutes: 7,
      content: `يبدأ كثير من متعلمي اللغة الإنجليزية رحلتهم بحماس كبير، لكن سرعان ما يفقدون الدافع بسبب اتباع طرق تقليدية لا تناسب طبيعة تعلم اللغات الحديثة. تعلم لغة جديدة ليس مجرد حفظ قواعد ومفردات، بل هو بناء عادة يومية تدريجية تراكمية، تشبه بناء أي مهارة أخرى.

الخطوة الأولى والأهم هي تحديد الهدف بوضوح. هل تتعلم الإنجليزية للسفر؟ للعمل؟ للدراسة؟ لكل هدف مسار مختلف من حيث المفردات والمهارات ذات الأولوية. المتعلم الذي يحدد هدفه بدقة يستطيع توجيه وقته ومجهوده نحو ما يحتاجه فعلًا، بدلًا من تشتيت جهده في محتوى عام غير موجّه.

من أهم الاستراتيجيات العملية التي أثبتت فعاليتها مع متعلمين حقيقيين:

الانغماس اليومي، ولو لمدة قصيرة. الاستماع لبودكاست أو مشاهدة حلقة مسلسل بالإنجليزية، حتى لو لم تفهم كل كلمة، يدرّب الأذن على إيقاع اللغة الطبيعي، وهذا أهم بكثير من ساعات طويلة متقطعة من الدراسة النظرية.

القراءة بمستوى مناسب. اختيار نصوص أعلى قليلًا من مستواك الحالي، وليس أعلى بكثير، يوسّع مفرداتك تدريجيًا دون إحباط.

التحدث من اليوم الأول، ولو مع نفسك. كثير من المتعلمين يؤجلون التحدث حتى "يصبحوا جاهزين"، لكن هذا التأجيل هو ما يمنعهم من التقدم فعليًا. التحدث، حتى بأخطاء، هو ما يبني الطلاقة الحقيقية.

تدوين المفردات الجديدة في جمل، لا ككلمات منفصلة. حفظ كلمة بمعزل عن سياقها يسهل نسيانها، بينما ربطها بجملة كاملة يجعلها أكثر رسوخًا في الذاكرة طويلة المدى.

من الأخطاء الشائعة التي يقع فيها المتعلمون، الانتقال المستمر بين مصادر ومناهج مختلفة دون الالتزام بمسار واحد لفترة كافية. الاستمرارية أهم من الكمال في تعلم اللغات، والمتعلم الذي يخصص عشرين دقيقة يوميًا لمدة سنة، يتقدم أكثر بكثير ممن يدرس بشكل مكثف لأسبوع ثم يتوقف لشهور.

التكنولوجيا الحديثة سهّلت الوصول لموارد تعلم عالية الجودة مجانًا أو بتكلفة منخفضة، من تطبيقات المحادثة مع متحدثين أصليين، إلى منصات الذكاء الاصطناعي التي تتيح تدريبًا تفاعليًا فوريًا على النطق والقواعد.

في النهاية، إتقان الإنجليزية ليس سباقًا قصيرًا، بل رحلة تراكمية تحتاج صبرًا واستمرارية أكثر من احتياجها لموهبة خاصة. أي شخص قادر على تعلم لغة جديدة بالنهج الصحيح والوقت الكافي، بغض النظر عن نقطة انطلاقه.`,
    },
    {
      category: getCategoryId('business'),
      title: 'كيف تصبح مترجمًا محترفًا وتربح من الترجمة أونلاين؟',
      slug: 'professional-translator-career-guide',
      description: 'دليل عملي لتحويل مهارة الترجمة إلى مصدر دخل حقيقي عبر الإنترنت، من بناء الأرشيف المهني إلى إيجاد أول عميل حقيقي.',
      metaKeywords: 'الربح من الترجمة, العمل كمترجم مستقل, الترجمة أونلاين, مهنة الترجمة عن بعد',
      image: 'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?auto=format&fit=crop&w=1200&q=80',
      minutes: 6,
      content: `مع اتساع سوق العمل الرقمي، أصبحت الترجمة واحدة من أكثر المهارات اللغوية القابلة للتحويل إلى مصدر دخل مستقل وحقيقي، سواء كعمل جانبي أو كمهنة كاملة عن بعد. لكن الانتقال من "أعرف لغتين" إلى "أربح من الترجمة فعليًا" يحتاج خطة واضحة، لا مجرد انتظار الفرص.

الخطوة الأولى هي بناء أرشيف أعمال يثبت مستوى جودتك، حتى قبل الحصول على أول عميل مدفوع. يمكن تحقيق ذلك بترجمة نصوص قصيرة في تخصصك المفضل، ونشرها كعينات تُظهر أسلوبك ودقتك.

الخطوة الثانية هي اختيار قنوات العمل المناسبة. هناك ثلاث قنوات رئيسية لمترجم مستقل يبدأ مسيرته:

منصات العمل الحر العالمية، والتي تتيح الوصول لعملاء دوليين، لكنها تتطلب صبرًا في البداية لبناء تقييمات وسمعة موثوقة.

مكاتب الترجمة، التي توفر تدفقًا مستمرًا من المشاريع لكن بأسعار أقل غالبًا مقارنة بالتعامل المباشر مع العملاء، وهي مناسبة جدًا لبناء الخبرة الأولى.

التواصل المباشر مع الشركات والعملاء، وهو الأصعب في البداية لكنه الأكثر ربحية على المدى الطويل، خاصة عند التخصص في مجال محدد كالترجمة القانونية أو التقنية.

الخطوة الثالثة هي التسعير الصحيح. كثير من المترجمين المبتدئين يبخسون أسعارهم خوفًا من خسارة العملاء، لكن هذا يخلق دورة صعبة من العمل الكثير بدخل قليل. الأفضل دراسة أسعار السوق في تخصصك، والتسعير بما يعكس قيمة الجودة التي تقدمها، ورفع السعر تدريجيًا مع تراكم الخبرة والتقييمات الإيجابية.

من العوامل التي تزيد فرص النجاح في هذا المجال، التخصص الدقيق. مترجم يقول "أنا مترجم عام" يتنافس مع آلاف المترجمين، بينما مترجم يقول "أنا متخصص في ترجمة العقود القانونية من الإنجليزية للعربية" يصبح خيارًا واضحًا لعميل يبحث تحديدًا عن هذه الخبرة.

كذلك، الالتزام بالمواعيد النهائية والتواصل الاحترافي مع العملاء، عاملان قد يبدوان بديهيين، لكنهما من أكثر أسباب فقدان العملاء المتكررين في هذا المجال، أكثر حتى من جودة الترجمة نفسها في بعض الأحيان.

أخيرًا، الاستثمار المستمر في الأدوات والمهارات، ومتابعة تطورات الذكاء الاصطناعي في الترجمة، يضمن للمترجم المستقل البقاء قادرًا على المنافسة وتقديم قيمة يصعب على الأدوات الآلية وحدها تعويضها.

النجاح في الترجمة كمصدر دخل مستقل ليس سريعًا، لكنه واقعي وقابل للتحقيق تمامًا لمن يتعامل معه كمشروع مهني جاد، لا كمحاولة عابرة.`,
    },
  ];

  for (const a of articles) {
    insertArticle.run(ahmed.id, a.category, a.title, a.slug, a.description, a.metaKeywords, a.content, a.image, a.minutes);
  }
  db.prepare('UPDATE writer_profiles SET article_count = article_count + ? WHERE user_id = ?').run(articles.length, ahmed.id);
